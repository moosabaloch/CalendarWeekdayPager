...
PagingViewControllerInfiniteDataSource where T: Hashable & Comparable {
  ...
  func pagingViewController<U>(
    ...
    func pagingViewController<U>(
        _ pagingViewController: PagingViewController<U>,
        realPagingItemBeforePagingItem item: U, isReal : Bool) -> U? {
    ...
    func pagingViewController<U>(
        _ pagingViewController: PagingViewController<U>,
        realPagingItemAfterPagingItem item: U , isReal : Bool) -> U? {
  ...
  func pagingViewController<U>(
    ...
    // Offset Index Implemnetation
    func pagingViewController<U>(
        _ pagingViewController: PagingViewController<U>,
        pagingItemBeforePagingItem item: U , withIndexOffset : Int) -> U? {
        
        guard let index = items.index(of: item as! T) else { return nil }
        if index > withIndexOffset {
            return items[index - withIndexOffset] as? U
        }
        return nil
    }
    ...
    func pagingViewController<U>(
        _ pagingViewController: PagingViewController<U>,
        pagingItemAfterPagingItem item: U, withIndexOffset : Int) -> U? {
        
        guard let index = items.index(of: item as! T) else { return nil }
        if index < items.count - (withIndexOffset) {
            return items[index + withIndexOffset] as? U
        }
        return nil
    }
