...
EMPageViewControllerDelegate where T: Hashable & Comparable {
    ...
    @objc private dynamic func handleSwipeGestureRecognizer(_ recognizer: UISwipeGestureRecognizer) {
        ...
        if recognizer.direction.contains(.left) {
            upcomingPagingItem = infiniteDataSource?.pagingViewController(self, pagingItemAfterPagingItem: currentPagingItem, withIndexOffset: 7)
        } else if recognizer.direction.contains(.right) {
            upcomingPagingItem = infiniteDataSource?.pagingViewController(self, pagingItemBeforePagingItem: currentPagingItem, withIndexOffset: 7)
        }
